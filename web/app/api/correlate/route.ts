...
export async function POST(req: NextRequest) {
  ...
  const slug = body.slug ?? req.nextUrl.searchParams.get("slug") ?? REPLAY_SLUG;

  if (!body.market || !body.evidence || !body.culture) {
    return NextResponse.json(await correlateForSlug(slug, asOf));
  }

  return NextResponse.json(
    await correlate(body.market, body.evidence, body.culture, asOf),
  );
}

/** P3 calls GET /api/correlate?mode=asof */
